		     ...
		     void *icmp_before, void *icmp_after, bool add)
{
	 ...
	 * itself.
	 */
	csum = bpf_csum_diff((__be32 *)&ph, add ? 0 : sizeof(ph), (__be32 *)&ph,
			     add ? sizeof(ph) : 0, 0);
 ...
 *  +--+---+---+---+---+---+---+---+---+---+---+---+---+---+---+---+---+
 */
static __always_inline bool v4addr_to_v6(__be32 addr4, struct in6_addr *addr6,
 ...
 */
static __always_inline bool v6addr_to_v4(const struct in6_addr *addr6,
					 int pref64_len, __be32 *addr4,
...
static int nat64_handle_v4(struct __sk_buff *skb, struct hdr_cursor *nh)
{
	...
	if (!v4addr_to_v6(iph->saddr, &dst_hdr.saddr, &config.v6_prefix, config.v6_plen))
		goto out;
...
static struct v6_addr_state *alloc_new_state(struct in6_addr *src_v6)
{
	...
	for (i = 0; i < 10; i++) {
		...
		if (__sync_val_compare_and_swap(&config.next_addr, next_addr,
	...
	if (bpf_map_update_elem(&v6_state_map, src_v6, &new_v6_state,
				BPF_NOEXIST))
		goto err;
...
static int nat64_handle_v6(struct __sk_buff *skb, struct hdr_cursor *nh)
{
	...
	if (!v6addr_to_v4(&ip6h->daddr, config.v6_plen, &dst_v4, &v6_prefix))
		goto out;

	if (cmp_v6addr(&v6_prefix, &config.v6_prefix)) {
		DBG("v6: dst address %pI6c not in configured prefix %pI6c/%u\n",
		    &ip6h->daddr, &config.v6_prefix, config.v6_plen);
		goto out;
	}
	...
	if (!dst_v4 || /* 0.0.0.0 */
	    (dst_v4 & bpf_htonl(0xFF000000)) ==
		    bpf_htonl(0x7F000000) || /* 127.x.x.x */
	    (dst_v4 & bpf_htonl(0xF0000000)) ==
		    bpf_htonl(0xe0000000)) { /* multicast */
	...
	if (!v6_state) {
		...
		bpf_map_update_elem(&v6_state_map, &ip6h->saddr, v6_state,
				    BPF_EXIST);
	...
	dst_hdr.tot_len =
		bpf_htons(bpf_ntohs(ip6h->payload_len) + sizeof(dst_hdr));
	...
	case IPPROTO_TCP:
	case IPPROTO_UDP:
		update_l4_checksum(skb, ip6h, &dst_hdr, dst_hdr.protocol,
				   false);
		break;
	default:
		break;
	...
	dst_hdr.check = csum_fold_helper(bpf_csum_diff(
		(__be32 *)&dst_hdr, 0, (__be32 *)&dst_hdr, sizeof(dst_hdr), 0));
